...
# possibilité 2 : on force les variables catégorielles en variables continues (version Marion)
bdd_expo_cat <- metadata %>%
  select(ident, 
         contains("BPS"), 
         contains("BUPA"), 
         contains("PFDoDA"),
  ...
  mutate(
    ...
    ch_BPS_total_cont_log_Y1 = log(ch_BPS_total_cont_Y1), 
    ...
    mo_BUPA_total_cont_log_t2 = log(mo_BUPA_total_cont_t2), 
    ...
    mo_BUPA_total_cont_log_t3 = log(mo_BUPA_total_cont_t3), 
    ...
    ch_BUPA_total_cont_log_Y1 = log(ch_BUPA_total_cont_Y1), 
    ...
    mo_PFDoDa_cont_log = log(mo_PFDoDa_cont), 
    ...
    mo_PFHxPA_cont_log = log(mo_PFHxPA_cont), 
    ...
    mo_PFTrDa_cont_log = log(mo_PFTrDa_cont)) %>%
  select(ident, 
         "ch_BPS_total_cont_Y1",      
         "ch_BPS_total_cont_log_Y1",
         "mo_BUPA_total_cont_t2",
         "mo_BUPA_total_cont_log_t2",
         "mo_BUPA_total_cont_t3",    
         "mo_BUPA_total_cont_log_t3",
         "ch_BUPA_total_cont_Y1",
...
bdd_exposure_2 <- metadata %>%           
  select(
    ident,
    all_of(phenols_vec_2), 
    all_of(pfas_vec))%>%           # variables d'exposition (déjà logtransformé)
  mutate_all(as.numeric)             # transformer toutes les expositions catégorielles en numérique 0, 1 

bdd_exposure_2 <- 
  left_join(bdd_exposure_2,
            bdd_expo_cat,
            by = "ident") %>%
  select(ident, 
         "mo_MEPA_total_i_cor_t2_ln", "mo_MEPA_total_i_cor_t3_ln", "ch_MEPA_total_i_cor_M2_ln", "ch_MEPA_conj_i_cor_Y1_ln",
         "mo_ETPA_total_i_cor_t2_ln", "mo_ETPA_total_i_cor_t3_ln", "ch_ETPA_total_cat_M2_2", "ch_ETPA_total_i_cor_Y1_ln",
         "mo_PRPA_total_i_cor_t2_ln", "mo_PRPA_total_i_cor_t3_ln", "ch_PRPA_total_cat_M2_2", "ch_PRPA_total_i_cor_Y1_ln",
         "mo_BUPA_total_cont_log_t2", "mo_BUPA_total_cont_log_t3", "ch_BUPA_total_cat_M2_2", "ch_BUPA_total_cont_log_Y1", 
         "mo_BPA_total_i_cor_t2_ln","mo_BPA_total_i_cor_t3_ln", "ch_BPA_conj_i_cor_M2_ln", "ch_BPA_conj_i_cor_Y1_ln",  
         "mo_BPS_total_cat_t2_2", "mo_BPS_total_cat_t3_2", "ch_BPS_total_cat_M2_2", "ch_BPS_total_cont_log_Y1",
...
keep_expo_2 <- colnames(bdd_exposure_2[, 2:48])
bdd_exposure_2[, keep_expo_2] <- lapply(bdd_exposure_2[, keep_expo_2], scale) # standardiser les expositions (diviser par sd)
colnames(bdd_exposure_2) <- c("ident", keep_expo_2)
...
## possibilité 2 : fenetres d'exposition confondues, avec les variables catégorielles transformées en numérique
bdd_bkmr_alpha_2 <- 
  list(bdd_outcomes_alpha, bdd_covariates_i, bdd_exposure_2) %>%
  reduce(left_join, by = "ident") %>%
  na.omit()

mixture_2 <- bdd_bkmr_alpha_2 %>% select(all_of(keep_expo_2)) %>% as.matrix()
covariates_2 <- bdd_bkmr_alpha_2 %>% select(all_of(keep_covar)) %>% as.matrix()
...
bkmr_not_hierar <- function(outcome) {
  set.seed(111)
  results_not_hierar_num <- kmbayes(    # modèle sans les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome, 
    Z = mixture_num, 
    X = covariates, 
  ...
  results_not_hierar_all <- kmbayes(    # modèle avec les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome, 
    Z = mixture_all, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
  ...
  results <- list(results_not_hierar_num, 
                  results_not_hierar_all)
  
  return(results)
}

bkmr_hierar <- function(outcome) {
  set.seed(111)
  results_hierar_num_family <- kmbayes(    # modèle sans les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome, 
    Z = mixture_num, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
    groups = c(1,1,1,1,    # methylparaben continue
               1,1,1,      # ethyparaben continue sauf M2
               1,1,1,      # propylparaben continue sauf M2
               # butylparaben (cat)
               1,1,1,1,    # bisphenol A continue
               # bisphenol S (cat)
               1,1,1,1,    # benzophenone 3 continue
               1,1,1,1,    # Triclosan continue
               2,2,2,2,2,2,2),   # 7 PFAS t2
    verbose = FALSE,       # if TRUE, la sortie intermédiaire résumant la progression de l'ajustement du modèle est imprimée
    varsel = TRUE)         # if TRUE, we can fit the model with variable selection and estimate the posterior inclusion probability (PIP) for each of the exposures zim
  
  results_hierar_all_family <- kmbayes(    # modèle avec les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome, 
    Z = mixture_all, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
    groups = c(1,1,1,1,    # methylparaben 
               1,1,1,1,    # ethyparaben 
               1,1,1,1,    # propylparaben 
               1,1,1,1,    # butylparaben 
               1,1,1,1,    # bisphenol A 
               1,1,1,1,    # bisphenol S 
               1,1,1,1,    # benzophenone 3 
               1,1,1,1,    # Triclosan 
               2,2,2,2,2,2,2,2,2,2,2,2,2,2,2),
    verbose = FALSE,       # if TRUE, la sortie intermédiaire résumant la progression de l'ajustement du modèle est imprimée
    varsel = TRUE)         # if TRUE, we can fit the model with variable selection and estimate the posterior inclusion probability (PIP) for each of the exposures zim
  
  results_hierar_num_window <- kmbayes(    # modèle sans les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome, 
    Z = mixture_num, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
    groups = c(1,1,2,2,    # methylparaben continue
               1,1,2,      # ethyparaben continue sauf M2
               1,1,2,      # propylparaben continue sauf M2
               # butylparaben (cat)
               1,1,2,2,    # bisphenol A continue
               # bisphenol S (cat)
               1,1,2,2,    # benzophenone 3 continue
               1,1,2,2,    # Triclosan continue
               3,3,3,3,3,3,3),   # 7 PFAS t2
    verbose = FALSE,       # if TRUE, la sortie intermédiaire résumant la progression de l'ajustement du modèle est imprimée
    varsel = TRUE)         # if TRUE, we can fit the model with variable selection and estimate the posterior inclusion probability (PIP) for each of the exposures zim
  
  results_hierar_all_window <- kmbayes(    # modèle avec les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome, 
    Z = mixture_all, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
    groups = c(1,1,2,2,    # methylparaben 
               1,1,2,2,      # ethyparaben 
               1,1,2,2,      # propylparaben 
               1,1,2,2,      # butylparaben 
               1,1,2,2,      # bisphenol A 
               1,1,2,2,      # bisphenol S 
               1,1,2,2,      # benzophenone 3 
               1,1,2,2,      # Triclosan 
  ...
  results <- list(results_hierar_num_family, 
                  results_hierar_all_family, 
                  results_hierar_num_window, 
                  results_hierar_all_window)
...
bkmr_sep_window <- function(mixture) {
  set.seed(111)
  results_specrich_sep_window <- kmbayes(    # modèle sans les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome_specrich, 
    Z = mixture, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
  ...
  results_shannon_sep_window <- kmbayes(    # modèle avec les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome_shannon, 
    Z = mixture, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
  ...
  results_faith_sep_window <- kmbayes(    # modèle avec les variables catégorielles "numérisées" / toutes fenetres confondues
    y = outcome_faith, 
    Z = mixture, 
    X = covariates, 
    iter = 50000,          # mettre 50 000
  ...
  results <- list(results_specrich_sep_window, 
                  results_shannon_sep_window, 
                  results_faith_sep_window)
...
TracePlot_group <- function(model_specrich, model_shannon, model_faith, titre){
  par(mfrow=c(3,3))
  TracePlot(fit = model_specrich, par = "beta") 
  TracePlot(fit = model_specrich, par = "sigsq.eps") 
  TracePlot(fit = model_specrich, par = "r", comp = 1) 
  TracePlot(fit = model_shannon, par = "beta") 
  TracePlot(fit = model_shannon, par = "sigsq.eps") 
  TracePlot(fit = model_shannon, par = "r", comp = 1) 
  TracePlot(fit = model_faith, par = "beta") 
  TracePlot(fit = model_faith, par = "sigsq.eps") 
  TracePlot(fit = model_faith, par = "r", comp = 1) 
...
pip_results_not_hierar <- function(bkmr_specrich, bkmr_shannon, bkmr_faith) {
  bkmr_pip_spechrich <- 
    ExtractPIPs(bkmr_specrich) %>% 
    as.data.frame() %>% 
...
# Model convergence ----
## BKMR not hierachical ----
TracePlot_group(bkmr_sperich_not_hierar_num, bkmr_shannon_not_hierar_num, bkmr_faith_not_hierar_num, 
                titre = "Modèles BKMR non hierarchiques, expo numériques uniquement (Specific richness, Shannon et Faith de haut en bas)")
TracePlot_group(bkmr_sperich_not_hierar_all, bkmr_shannon_not_hierar_all, bkmr_faith_not_hierar_all, 
                titre = "Modèles BKMR non hierarchiques, toutes expositions (Specific richness, Shannon et Faith de haut en bas)")

## BKMR hierachical ----
TracePlot_group(bkmr_sperich_hierar_num_family, bkmr_shannon_hierar_num_family, bkmr_faith_hierar_num_family, 
                titre = "Modèles BKMR hierarchical 2 groupes (phénols/pfas), expo numériques uniquement (Specific richness, Shannon et Faith de haut en bas)")
TracePlot_group(bkmr_sperich_hierar_all_family, bkmr_shannon_hierar_all_family, bkmr_faith_hierar_all_family, 
                titre = "Modèles BKMR hierarchical 2 groupes (phénols/pfas), toutes expositions (Specific richness, Shannon et Faith de haut en bas)")

TracePlot_group(bkmr_sperich_hierar_num_window, bkmr_shannon_hierar_num_window, bkmr_faith_hierar_num_window, 
                titre = "Modèles BKMR hierarchical 3 groupes (phenols prénat, phénols postnat et pfas), expo numériques uniquement (Specific richness, Shannon et Faith de haut en bas)")
TracePlot_group(bkmr_sperich_hierar_all_window, bkmr_shannon_hierar_all_window, bkmr_faith_hierar_all_window, 
                titre = "Modèles BKMR hierarchical 3 groupes (phenols prénat, phénols postnat et pfas), toutes expositions (Specific richness, Shannon et Faith de haut en bas)")
...
TracePlot_group(bkmr_sperich_hierar_all_window_5, bkmr_shannon_hierar_all_window_5, bkmr_faith_hierar_all_window_5, 
                titre = "Modèles BKMR hierarchical 5 groupes (phenols t2, t3, M2, Y1 et pfas), toutes expositions (Specific richness, Shannon et Faith de haut en bas)")
...
bkmr_risks_singvar_results_specrich_all_window <- risks_singvar(bkmr_sperich_hierar_all_window, outcome_specrich, mixture_all)
bkmr_risks_singvar_results_shannon_all_window <- risks_singvar(bkmr_shannon_hierar_all_window, outcome_shannon, mixture_all)
bkmr_risks_singvar_results_faith_all_window <- risks_singvar(bkmr_faith_hierar_all_window, outcome_faith, mixture_all)

bkmr_risks_singvar_results_specrich_all_familly <- risks_singvar(bkmr_sperich_hierar_all_familly, outcome_specrich, mixture_all)
bkmr_risks_singvar_results_shannon_all_familly <- risks_singvar(bkmr_shannon_hierar_all_familly, outcome_shannon, mixture_all)
bkmr_risks_singvar_results_faith_all_familly <- risks_singvar(bkmr_faith_hierar_all_familly, outcome_faith, mixture_all)

bkmr_risks_singvar_results_specrich_num_window <- risks_singvar(bkmr_sperich_hierar_num_window, outcome_specrich, mixture_num)
bkmr_risks_singvar_results_shannon_num_window <- risks_singvar(bkmr_shannon_hierar_num_window, outcome_shannon, mixture_num)
bkmr_risks_singvar_results_faith_num_window <- risks_singvar(bkmr_faith_hierar_num_window, outcome_faith, mixture_num)

bkmr_risks_singvar_results_specrich_num_familly <- risks_singvar(bkmr_sperich_hierar_num_familly, outcome_specrich, mixture_num)
bkmr_risks_singvar_results_shannon_num_familly <- risks_singvar(bkmr_shannon_hierar_num_familly, outcome_shannon, mixture_num)
bkmr_risks_singvar_results_faith_num_familly <- risks_singvar(bkmr_faith_hierar_num_familly, outcome_faith, mixture_num)
...
bkmr_risks_singvar_specrich_num_sep_window_hierar_t2 <- risks_singvar(bkmr_sperich_sep_window_hierar_num_t2, outcome_specrich, mixture_t2_num)
bkmr_risks_singvar_shannon_num_sep_window_hierar_t2 <- risks_singvar(bkmr_shannon_sep_window_hierar_num_t2, outcome_shannon, mixture_num)
bkmr_risks_singvar_faith_num_sep_window_hierar_t2 <- risks_singvar(bkmr_faith_sep_window_hierar_num_t2, outcome_faith, mixture_num)

bkmr_risks_singvar_specrich_num_sep_window_t3 <- risks_singvar(bkmr_sperich_sep_window_t3_num, outcome_specrich, mixture_t2_num)
bkmr_risks_singvar_shannon_num_sep_window_t3 <- risks_singvar(bkmr_shannon_sep_window_t3_num, outcome_shannon, mixture_t3_num)
bkmr_risks_singvar_faith_num_sep_window_t3 <- risks_singvar(bkmr_faith_sep_window_t3_num, outcome_faith, mixture_t3_num)
