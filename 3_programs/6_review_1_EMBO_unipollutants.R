...
bdd_q.value <- results_multi %>% filter(analysis %in% "exploratory") %>%
  filter(outcome %in% c("Specific richness", "Shannon diversity", "Faith phylogenetic diversity"))%>%
  filter(row_to_choose %in% "yes") %>%
  select(model_type, 
         analysis, 
         outcome, 
         exposure, 
         ...
         p.value)%>%
  mutate(
    q.value = p.adjust(p.value, method = "fdr"))   # length(p_value_final) = 27 expo x 3 outcomes = 81)

results_multi <-  left_join(results_multi, 
            bdd_q.value, 
            by = c("model_type", 
                   "analysis", 
                   "outcome", 
...
results_multi <- results_multi %>%
  mutate(
    q_value_shape = case_when(q.value < 0.1 ~ "q.value <0.1",
                              q.value > 0.1 ~ "q.value >0.1"), 
    q_value_shape = fct_relevel(q_value_shape,
                                "q.value >0.1", 
                                "q.value <0.1")
  )

q.values_alpha_explo <- results_multi %>% 
  filter(outcome %in% c("Specific richness", "Shannon diversity", "Faith phylogenetic diversity")) %>%
  filter(analysis == "exploratory") %>%
  filter(row_to_choose == "yes") %>%
  select(outcome, exposure, estimate, conf.low, conf.high, p.value, q.value)
